...
def deconv_apg_tv(im,psf,scale,mu,conv_med,max_iter):
    ...
    for i in range(max_iter):
        ...
        for k in range(10):
            w_new = w - a*grad
            x_new,l,r = deconv_admm(np.reshape(w_new,hr_im_sz),[[1]],mu)
            x_new = x_new.ravel()
...
def deconv(im,psf,scale,mu,conv_med = ['dilation','dilation'], deconv_med='ADMM_TV',max_iter = 50):
    ...
    if deconv_med == 'ADMM_TV':
        if scale[0]*scale[1] == 1:
            out, loss,rdiff = deconv_admm(im,psf,mu)
        else:
            print('ADMM_TV does not support upscaling; scale = [1,1] is required.')
    elif deconv_med == 'APG_TV':
        out,loss,rdiff = deconv_apg_tv(im,psf,scale,mu,conv_med = conv_med,max_iter=max_iter)
    elif deconv_med == 'APG_BM3D':
        out,loss,rdiff = deconv_apg_bm3d(im,psf,scale,mu,conv_med = conv_med,max_iter=max_iter)
    return out, loss, rdiff
